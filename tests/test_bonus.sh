...
@test "PARAMS: 5 params OK" {
    run ../pipex "" "" "" "" "" ""
    [[ "$status" != 0 ]]
}

@test "PARAMS: 6 params OK" {
    run ../pipex "" "" "" "" "" "" ""
    [[ "$status" != 0 ]]
}
...
@test "EMPTY COMMANDS: empty 4 / 4" {
    setup_io
    echo hola > "$input_file"

    myrun -1 ../pipex "$input_file" "cat" "cat" "cat" "" "$output_file"
...
@test "FILE INPUT: wrong permission output file" {
    ...
    myrun ../pipex "$input_file" "grep hola" "rev" "rev" "sort" "$output_file"
...
@test "FILE INPUT: input file doesn't exist" {
    ...
    myrun -0 ../pipex "$input_file" "" "cat" "cat" "cat" "$output_file"
...
@test "COMMAND PARSING: heredoc is enabled" {
    setup_io
    cat << EOF | ../pipex "here_doc" cat cat cat "$output_file"
hola como
estas el mio
cat
EOF
...
@test "COMMAND PARSING: heredoc warns if delim not found" {
    setup_io
    cat << EOF | ../pipex "here_doc" cat cat cat "$output_file" 2>"$input_file"
hola como
estas el mio
EOF
    [[ $? -eq 0 ]]
...
EOF
    ...
    cat "$input_file" >&2
    [[ "$(cat "$input_file")" == "warning: here-document delimited by end-of-file (wanted \`cat')" ]]
}
